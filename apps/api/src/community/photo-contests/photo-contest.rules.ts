...
import type { CandidateRecord, ContestRecord, EligibleTripRecord } from './photo-contest.types.js';
...
export function acceptsEntries(contest: ContestRecord, now: Date): boolean {
  return contest.status === 'OPEN' && Date.parse(contest.startsAt) <= now.getTime() &&
    now.getTime() < Date.parse(contest.endsAt);
}
export function assertOpen(contest: ContestRecord, now: Date): void {
...
export function assertSubmission(trip: EligibleTripRecord | null, userId: string, countryId: string): asserts trip is EligibleTripRecord & { coverStoragePath: string } {
...
export function selectWinner(candidates: CandidateRecord[]): string | null {
  return [...candidates].sort((a, b) => b.votes - a.votes ||
    (a.createdAt < b.createdAt ? -1 : a.createdAt > b.createdAt ? 1 : 0) ||
    (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))[0]?.id ?? null;
}

export function currentWinnerContest(history: ContestRecord[]): ContestRecord | null {

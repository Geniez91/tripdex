import type { CountryMemoryDto, PhotoContestDto } from './photo-contest.dto.js';
import type { ContestDetailRecord, MemoryRecord } from './photo-contest.types.js';
import { acceptsEntries } from './photo-contest.rules.js';

export class PhotoContestMapper {
  static detail(record: ContestDetailRecord, urls: Map<string, string | null>, now: Date): PhotoContestDto {
  ...
  static memory(record: MemoryRecord, imageUrl: string | null): CountryMemoryDto | null {
  ...
  static activity(contest: PhotoContestDto) {

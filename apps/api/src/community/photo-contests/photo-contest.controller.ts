...
export class PhotoContestController {
  ...
  @UseGuards(AuthGuard)
  @Header('Cache-Control', 'no-store')
  participation(@Param('contestId') id: string, @CurrentUser() user: TripDexUser) {
    return this.contests.participation(id, user.id);
  }
  @Post('photo-contests/:contestId/submissions')
  @UseGuards(AuthGuard)
  @HttpCode(204)
  submit(@Param('contestId') id: string, @CurrentUser() user: TripDexUser, @Body(SubmissionPipe) input: SubmissionInput) {
    return this.contests.submit(id, user.id, input.tripId);
  }
  @Put('photo-contests/:contestId/vote')
  @UseGuards(AuthGuard)
  @HttpCode(204)
  vote(@Param('contestId') id: string, @CurrentUser() user: TripDexUser, @Body(VotePipe) input: VoteInput) {
